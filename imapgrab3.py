...
# Updated to work with Python 3 by Christian König-Kersting, 2020
# Tested with python 3.7.7 and getmail 5.14
...
# Usage:
#  imapgrab3.py [-ldaSvMB] [-s] SERVER [-P] PORT [-u] USERNAME [-p] PASSWORD [-m] "BOX1,BOX2,..." [-f] DIRECTORY
#    Possible arguments:
...
#       imapgrab3.py -l -s imap.example.com -u username -p password
#  2) Download "box1" and "box2" from server imap.example.com (save "box1.mbox" and "box2.mbox")
#       imapgrab3.py -d -s imap.example.com -u username -p password -m "box1, box2"
#  3) Download all mailboxes except "box3"
#       imapgrab3.py -d -s imap.example.com -u username -p password -m "_ALL_, -box3"
#  4) Download all Gmail custom labels and INBOX (none of the [Gmail]* mailboxes)
#       imapgrab3.py -d -S -s imap.gmail.com -u username -p password -m "_ALL_, -_Gmail_"
#  5) Download Gmail label "receipts"
#       imapgrab3.py -d -S -s imap.gmail.com -u username -p password -m "receipts"
...
def imapgrab():
    ...
    ig_cmd.add_option("-m", "--mailboxes", dest="mailboxes",
                      help="Comma separated list of mailboxes to download (i.e. \"Box1, Box2, Box3\") ('{,}' for non-separating commas) ('_ALL_' for all mailboxes, '-Box1' to except Box1 from _ALL_) (required for -d)")
...
def IG_list_mailboxes(ig_options):
    ...
    for i in imap_list[1]:
        ig_list_raw.append(i.split(b'" ', 1)[1])
...
def IG_backup_mail(ig_options):
    ...
    if (ig_options.debug is True): print(
        "DEBUG_015: removed non-separating commas: \"" + ig_download_str_rm_comma + "\"")
    ...
    if len(ig_all_found) != len(ig_download_list):
        ...
        print(
            "ERROR: Some of the mailboxes you selected are not available for download. Use list option (-l or --list) to see availabe mailboxes.")
        return
    ...
    if ig_options.folder is None:
        ...
        if (ig_options.debug is True): print(
            "DEBUG_020: no -f parsed, using current directory: " + ig_options.folder + os.sep + "imapgrab")
    ...
    if os.path.exists(ig_options.folder) is not True:
        ...
        if (ig_options.debug is True): print(
            "DEBUG_021: folder does not exist, creating new folder in " + ig_options.folder)
            ...
            for f in range(0, len(folders) - 1):
                ...
                if os.path.exists(fpath) is not True:
                    ...
                    if (ig_options.verbose is True): print(
                        "\"" + folders[f] + "\" folder doesn't exist, creating folder")
                    os.mkdir(fpath)
        else:
            fpath = ig_options.folder
            filename = i_str
        ...
        if ig_options.maildir is True:
            ...
            for f in maildir_directories:
                if os.path.isdir(fpath + os.sep + filename + os.sep + f) is not True:
                    if (ig_options.verbose is True): print(
                        "Maildir directory doesn't exist, creating \"" + fpath + os.sep + filename + os.sep + f + "\"")
                    if (ig_options.debug is True): print(
                        "DEBUG_025: \"" + fpath + os.sep + filename + os.sep + f + "\" doesn't exist, creating")
                    os.mkdir(fpath + os.sep + filename + os.sep + f)
                else:
                    if (ig_options.verbose is True): print(
                        "Maildir directory already exists, using \"" + fpath + os.sep + filename + os.sep + f + "\"")
                    if (ig_options.debug is True): print(
    ...
    for mailbox in ig_download_list:
        ...
        if os.sep in mailbox_str:  # Check to see if there are folders in mailbox name
            folders = mailbox_str.split(os.sep)
            filename = folders[-1]
            path_from_folder = ""
        ...
        # Call getmail command
        if (ig_options.verbose is True): print(
            "Calling getmail to retrieve mail for mailbox \"" + ig_options.folder + os.sep + path_from_folder + os.sep + filename + "\"")
        if (ig_options.quiet is not True): print("Downloading mailbox \"" + mailbox_str + "\"")
        if (ig_options.debug is True): print(
            "DEBUG_027: calling \"" + cmd + " -g " + ig_options.folder + " -r " + rc_file_loc + "\"")
